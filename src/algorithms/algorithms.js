...
const shellSort = async(data,setData,time,active,setActive)=>{
    for
    (let gap = Math.floor(data.length/2) ; gap >0 ; gap=Math.floor(gap/2)){
...
const merge = async (arr,left,middle,right,data,setData,time ,active,setActive)=>{
    ...
    // creation of left and right array
...
export {bubbleSort,insertionSort,beginMergeSort as mergeSort,beginQuickSort as quickSort,shellSort,beginSort as sort}